...
const form = () => {
  ...
  phoneMask();
  telValidate();
  setupFormListener('.application__form', '.application__form .submit-btn');
  textareaValidate();
  ...
  function textareaValidate() {
    ...
    function textareaCount(item) {
      ...
      const wrap = item.closest('.form__textarea-wrap');
      const span = wrap.querySelector('.form__textarea-count .current');
      const length = +wrap.querySelector('.form__textarea-count .all').textContent;
      ...
      span.textContent = textareaValue.length;
    }
